        ...
        do {
            ...
            if (payTxAllocResult === null) {
                ...
                if (!payTxExpAddrsRefunded && Catenis.ctnHubNode.checkPayTxExpenseFundingBalance()) {
                    ...
                    Catenis.logger.WARN('Not enough UTXOs available to pay for send message tx expense. Refunding pay tx expense addresses to try again');
                    payTxExpAddrsRefunded = true;
                    retryPayTxExpense = true;
                }
                else {
                    // Log error condition and throw exception
                    Catenis.logger.ERROR('No UTXO available to be allocated to pay for send message tx expense');
                    throw new Meteor.Error('ctn_send_msg_no_utxo_pay_tx_expense', 'No UTXO available to be allocated to pay for send message tx expense');
                }
            }
        }
