...
//console.log('[BillingReportTemplate.js]: This code just ran.');
...
// Import template UI
import './BillingReportTemplate.html';

// Import dependent templates
import './BillingEntryTemplate.js';
...
function loadReport(template) {
    ...
    if (template.clientBillingReporSubs) {
        template.clientBillingReporSubs.stop();
    }
    
    template.clientBillingReporSubs = template.subscribe(
        'clientBillingReport',
...
Template.billingReport.onCreated(function () {
    ...
    this.clientDevicesSubs = this.subscribe('clientDevices', this.data.client_id, true, () => {
        initFilter(this);
        loadReport(this);
    });
    this.paidServiceNamesSubs = this.subscribe('paidServiceNames');
});

Template.billingReport.onDestroyed(function () {
    if (this.clientRecordSubs) {
        this.clientRecordSubs.stop();
    }

    if (this.clientBillingReporSubs) {
        this.clientBillingReporSubs.stop();
    }
    ...
    if (this.paidServiceNamesSubs) {
        this.paidServiceNamesSubs.stop();
    }
});

Template.billingReport.events({
...
Template.billingReport.helpers({
    client() {
