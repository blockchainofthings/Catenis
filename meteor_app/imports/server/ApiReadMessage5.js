...
//console.log('[ApiReadMessage5.js]: This code just ran.');
...
export function readMessage5() {
    try {
        ...
        if (typeof this.queryParams.encoding === 'string' && isValidMsgEncoding(this.queryParams.encoding)) {
            encoding = this.queryParams.encoding;
        }
        else if (this.queryParams.encoding !== undefined) {
            Catenis.logger.DEBUG('Invalid \'encoding\' parameter for GET \'messages/:messageId\' API request', this.queryParams);
            invalidParams.push('encoding');
        }

        let continuationToken;

        if (typeof this.queryParams.continuationToken === 'string' && this.queryParams.continuationToken.length > 0) {
        ...
        try {
            readResult = this.user.device.readMessage4(this.urlParams.messageId, encoding, continuationToken, dataChunkSize, async);
        }
        catch (err) {
            let error;
