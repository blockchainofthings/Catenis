/**
 * Created by claudio on 2020-06-16
 */

//console.log('[OwnedDomainsTemplate.js]: This code just ran.');
...
// Import template UI
import './OwnedDomainsTemplate.html';
...
Template.ownedDomains.onCreated(function () {
    ...
    this.ownedDomainsSubs = this.subscribe('ownedDomains', this.data.client_id);
});

Template.ownedDomains.onDestroyed(function () {
    if (this.clientRecordSubs) {
        this.clientRecordSubs.stop();
    }

    if (this.ownedDomainsSubs) {
        this.ownedDomainsSubs.stop();
    }
...
Template.ownedDomains.events({
    ...
    'submit #frmAddDomain'(event, template) {
        ...
        if ((domainInfo = validateAddDomainFormData(form, errMsgs))) {
            ...
            Meteor.call('addClientOwnedDomain', template.data.client_id, domainInfo.name, (error) => {
                if (error) {
    ...
    'submit #frmDeleteDomain'(event, template) {
        ...
        Meteor.call('deleteClientOwnedDomain', template.data.client_id, form.domainName.value, (error) => {
            if (error) {
    ...
    'submit #frmVerifyDomain'(event, template) {
        ...
        Meteor.call('verifyClientOwnedDomain', template.data.client_id, form.domainName.value, (error, success) => {
            if (error) {
                // Display error message
                template.state.set('verifyDomainErrMsgs', [
                    error.toString()
