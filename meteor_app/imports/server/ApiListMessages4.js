...
//console.log('[ApiListMessages4.js]: This code just ran.');
...
export function listMessages4() {
    try {
        ...
        // action param
        if (isValidMsgAction(this.queryParams.action)) {
            if (this.queryParams.action !== 'any') {
                filter.action = this.queryParams.action;
            }
        }
        else if (this.queryParams.action !== undefined) {
            Catenis.logger.DEBUG('Invalid \'action\' parameter for GET \'messages\' API request', this.queryParams);
            invalidParams.push('action');
        }

        // direction param
        if (isValidMsgDirection(this.queryParams.direction)) {
            if (this.queryParams.direction !== 'any') {
        ...
        // readState param
        if (isValidMsgReadState(this.queryParams.readState)) {
            if (this.queryParams.readState !== 'any') {
                filter.readState = this.queryParams.readState;
            }
        }
        else if (this.queryParams.readState !== undefined) {
            Catenis.logger.DEBUG('Invalid \'readState\' parameter for GET \'messages\' API request', this.queryParams);
            invalidParams.push('readState');
        }
